...
import { exportDatabase, importDatabase, getTasks,getDefects, addTask,addDefect, updateTask,updateDefect, getActivities, addActivity } from "../lib/database";
...
type Severity = "critical" | "major" | "minor" | "blocker";
type NavItem =
...
interface Defect{
  ...
  jiraTicket: string;  jiraLink?: string;
  severity: 'critical'|'major'|'minor'|'blocker';
  summary: string;
  status: 'open'|'in-progress'|'resolved'|'closed'|'reopened';
  project: string;  assignee: string;  createdAt: string;
  rca: string; workaround:string; lessonsLearned:string; description: string; 
}
interface Template {
...
const SAMPLE_DEFECTS: Defect[] = [
  { id: "1", jiraTicket: "BUG-1042", summary: "Checkout fails when applying multiple discount codes", severity: "critical", status: "open", project: "E-Commerce v3.2", assignee: "Dev: Arjun Mehta", foundIn:"QA",createdAt: "2026-06-25", rca: "Race condition in discount validation service when processing concurrent coupon API calls.", workaround: "Apply coupons one at a time with a 2-second delay between applications.", lessonsLearned: "Add concurrency tests for all discount/promo flows going forward.", description: "sample"},
  { id: "2", jiraTicket: "BUG-1038", summary: "PDF invoice generation fails for orders >100 items", severity: "major", status: "in-progress", project: "E-Commerce v3.2", assignee: "Dev: Priya Nair",foundIn:"Staging", createdAt: "2026-06-23", rca: "Memory overflow in PDF library ï¿½ no pagination for large item sets.", workaround: "Generate invoices manually via admin panel for orders exceeding 100 items.", lessonsLearned: "Test boundary conditions for all document generation functions." ,description: "sample"},
  { id: "3", jiraTicket: "BUG-1031", summary: "OAuth token refresh fails silently on mobile Safari", severity: "major", status: "resolved", project: "Auth Service 2.1", assignee: "Dev: Ravi Shankar", foundIn:"Prod",createdAt: "2026-06-20", rca: "Safari blocks third-party cookies by default; token refresh relied on cookie-based session.", workaround: "Users must re-login after token expiry on mobile Safari.", lessonsLearned: "Always test auth flows on Safari with default ITP settings enabled.",description: "sample" },
  { id: "4", jiraTicket: "BUG-1027", summary: "Dashboard charts not loading on slow connections (<3G)", severity: "minor", status: "open", project: "Analytics Dashboard", assignee: "Dev: Sneha Patel",foundIn:"Dev", createdAt: "2026-06-18", rca: "No lazy loading or skeleton states ï¿½ charts block render on slow network.", workaround: "None ï¿½ charts fail to load for users on slow connections.", lessonsLearned: "Implement skeleton loading states for all chart components.",description: "sample" },
  { id: "5", jiraTicket: "BUG-1019", summary: "Export CSV truncates rows > 10,000 records", severity: "major", status: "closed", project: "Analytics Dashboard", assignee: "Dev: Kiran Rao",foundIn:"QA", createdAt: "2026-06-14", rca: "CSV export hard-coded limit of 10k rows; not documented in UI.", workaround: "Use date-range filters to export in batches under 10k rows.", lessonsLearned: "Document all export limits in UI; implement streaming for large exports.", description: "sample"},
  { id: "6", jiraTicket: "BUG-1055", summary: "Search autocomplete returns stale results after profile update", severity: "minor", status: "reopened", project: "E-Commerce v3.2", assignee: "Dev: Arjun Mehta",foundIn:"QA", createdAt: "2026-06-26", rca: "Redis cache not invalidated on profile update.", workaround: "Hard refresh browser to clear local cache.", lessonsLearned: "Cache invalidation strategy must be part of feature design review.", description: "sample"},
];
...
const severityCfg: Record<Severity, { label: string; cls: string }> = {
  ...
  blocker: { label: "Trivial", cls: "bg-slate-500/20 text-slate-400 border border-slate-500/30" },
};
...
function Header({ dark, onToggleDark, searchQuery, onSearch, onQuickAdd }: {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `qa-nexus-backup-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
      URL.revokeObjectURL(url);
      setBackupStatus("Backup downloaded.");
    } catch {
      setBackupStatus("Export failed. Check console for details.");
    }
  };
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBackupStatus("Data restored. Reload the app to see changes.");
    } catch {
      setBackupStatus("Import failed. Make sure the file is a valid backup.");
    } finally {
      e.target.value = "";
    }
  };
  ...
  return (
    <header className="h-14 bg-background border-b border-border flex items-center gap-3 px-5 flex-shrink-0">
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        ...
        <button
          onClick={() => { setImportExportOpen(true); setBackupStatus(null); }}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-semibold transition-colors"
        >
          <Upload className="w-3.5 h-3.5" /> Backup
        </button>
      </div>
      ...
      <Dialog open={importExportOpen} onOpenChange={setImportExportOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Export all local data to a JSON file, or restore from a previous backup.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-3 py-2">
            <button
              onClick={handleExport}
              className="flex items-center justify-center gap-2 px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-medium transition-colors"
            >
              <Download className="w-4 h-4" /> Export data
            </button>
            <label className="flex items-center justify-center gap-2 px-3 py-2 bg-accent hover:bg-accent/70 rounded-lg text-sm font-medium transition-colors cursor-pointer">
          ...
          <DialogFooter>
            <DialogClose asChild>
              <button className="px-3 py-1.5 rounded-lg text-xs font-medium bg-accent hover:bg-accent/70 transition-colors">Close</button>
            </DialogClose>
          </DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            {openDefects.slice(0, 4).map(d => (
              <div key={d.id} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-muted/60 transition-colors">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] font-mono text-indigo-400">{d.jiraTicket}</div>
...
function DefectsPage({ defects, onAdd, onUpdate }: { defects: Defect[]; onAdd: (d: Omit<Defect, "id">) => void; onUpdate: (id: string, u: Partial<Defect>) => void }) {
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-[13px] text-muted-foreground mt-0.5">{defects.filter(d => d.status === "open").length} open  {defects.filter(d => d.severity === "critical" && d.status === "open").length} critical</p>
          </div>
          <button
  onClick={() => setShowAdd(true)}
  className="flex items-center gap-1.5 px-3 py-1.5 bg-red-600 hover:bg-red-500 text-white rounded-lg text-[13px] font-semibold transition-colors"
>
            ...
            <tbody>
              {filtered.map(d => (
                <tr key={d.id} onClick={() => setSelected(selected?.id === d.id ? null : d)} className={`border-b border-border last:border-0 hover:bg-accent/30 cursor-pointer transition-colors ${selected?.id === d.id ? "bg-red-600/5" : ""}`}>
                  <td className="px-4 py-3 text-xs font-mono text-indigo-400 font-semibold">{d.jiraTicket}</td>
  ...
  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
    <div className="bg-popover border border-border rounded-xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
      <div className="flex items-center justify-between mb-5">
        <h3 className="font-semibold">Report Defect</h3>
        <button onClick={() => setShowAdd(false)}><X className="w-4 h-4 text-muted-foreground" /></button>
      </div>
      <div className="space-y-3">
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Jira Ticket *</label>
          <input className="w-full bg-accent/50 border border-border rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:ring-1 focus:ring-indigo-500" placeholder="BUG-1042" value={draft.jiraTicket || ""} onChange={e => setDraft({ ...draft, jiraTicket: e.target.value })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Jira Link</label>
          <input className="w-full bg-accent/50 border border-border rounded-lg px-3 py-2 text-[13px] focus:outline-none" placeholder="https://..." value={draft.jiraLink || ""} onChange={e => setDraft({ ...draft, jiraLink: e.target.value })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Summary *</label>
          <input className="w-full bg-accent/50 border border-border rounded-lg px-3 py-2 text-[13px] focus:outline-none" placeholder="Short defect summary" value={draft.summary || ""} onChange={e => setDraft({ ...draft, summary: e.target.value })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Assignee *</label>
          <input className="w-full bg-accent/50 border border-border rounded-lg px-3 py-2 text-[13px] focus:outline-none" placeholder="Assigned to" value={draft.assignee || ""} onChange={e => setDraft({ ...draft, assignee: e.target.value })} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Severity</label>
            <select className="w-full bg-accent/50 border border-border rounded-lg px-3 py-2 text-[13px] focus:outline-none" value={draft.severity} onChange={e => setDraft({ ...draft, severity: e.target.value as Defect["severity"] })}>
              {(["critical", "major", "minor", "blocker"] as const).map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Found In</label>
            <select className="w-full bg-accent/50 border border-border rounded-lg px-3 py-2 text-[13px] focus:outline-none" value={draft.foundIn} onChange={e => setDraft({ ...draft, foundIn: e.target.value as Defect["foundIn"] })}>
              {(["Dev", "QA", "Staging", "Prod"] as const).map(f => <option key={f} value={f}>{f}</option>)}
            </select>
          </div>
        </div>
        <div>
        ...
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Description</label>
          <textarea rows={3} className="w-full bg-accent/50 border border-border rounded-lg px-3 py-2 text-[13px] focus:outline-none resize-none" placeholder="Steps to reproduce" value={draft.description || ""} onChange={e => setDraft({ ...draft, description: e.target.value })} />
        </div>
      </div>
      <div className="flex gap-2 mt-4">
        <button onClick={() => setShowAdd(false)} className="flex-1 py-2 bg-accent/60 hover:bg-accent rounded-lg text-[13px] transition-colors">Cancel</button>
        <button
          onClick={() => {
            if (draft.jiraTicket && draft.summary) {
              onAdd({
                ...
                status: "open",
                project: draft.project || SAMPLE_PROJECTS[0].name,
                foundIn: draft.foundIn || "QA",
              ...
              setShowAdd(false);
              setDraft({ severity: "minor", status: "open", project: "E-Commerce v3.2", foundIn: "QA" });
            }
          }}
          className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-[13px] font-semibold transition-colors"
        >Add Defect</button>
...
export default function App() {
  ...
  try {
    ...
    const mappedTasks: Task[] = dbTasks.map(t => ({ ...t, id: String(t.id) }));
    setTasks(mappedTasks);

    const dbActivities = await getActivities();
    const mappedActivities: ActivityEntry[] = dbActivities.map(a => ({ ...a, id: String(a.id) }));
    setActivities(mappedActivities);

    const dbDefects = await getDefects();
    const mappedDefects: Defect[] = dbDefects.map((d: any) => ({
      id: String(d.id),
      foundIn: d.foundIn,
      jiraTicket: d.jiraTicket,
      jiraLink: d.jiraLink,
      severity: d.severity ?? "minor",
      summary: d.summary ?? d.title ?? "No summary provided",
      status: d.status ?? "open",
      project: d.project ?? "Unknown",
      assignee: d.assignee ?? "Unassigned",
      createdAt: d.createdAt ?? d.updatedAt ?? formatTimestamp(new Date()),
      rca: d.rca ?? "",
    ...
    setDefects(SAMPLE_DEFECTS); // fallback only, matches existing pattern
  }
};
  ...
  // Defects handlers (basic optimistic handlers to satisfy component props)
  const handleAddDefect = async (d: Omit<Defect, "id">) => {
  try {
    const newId = await addDefect(d);
    const newDefect: Defect = { ...d, id: String(newId) };
    setDefects(prev => [...prev, newDefect]);
  } catch (error) {
    console.error('Failed to add defect to database:', error);
  }
};

const handleUpdateDefect = async (id: string, u: Partial<Defect>) => {
  try {
    const { id: _unused, ...updates } = u; // drop id, it's passed separately
    await updateDefect(Number(id), updates);
    setDefects(prev => prev.map(d => d.id === id ? { ...d, ...u } : d));
  } catch (error) {
    console.error('Failed to update defect in database:', error);
  }
};
  ...
  const renderPage = () => {
    switch (nav) {
      ...
      case "defects": return <DefectsPage defects={defects} onAdd={handleAddDefect} onUpdate={handleUpdateDefect}/>;
